...
- Linux 还需安装 `libsecret-1-dev`、`libwebkit2gtk-4.1-dev` 构建依赖，
  以及对应的 `libsecret-1-0`、`libwebkit2gtk-4.1-0` 运行时依赖，分别用于
  安全存储 Bot API 密钥的加密主密钥与 MCP 凭据，以及运行 HTML 产物预览
...
Bot API 密钥会先加密再随助手配置写入本地 SQLite，加密主密钥保存在操作系统安全凭据存储中；
请妥善保护设备和应用数据。
